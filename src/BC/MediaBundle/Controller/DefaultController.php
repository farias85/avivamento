...
class DefaultController extends Controller {
    ...
    public function fileSendAction($type, $entityName, $entityId, $isActive) {
        ...
        if (!empty($_FILES['file'])) {
            ...
            try {
                $media = $this->get('bc.media.manager')->asyncFileSend($file, $type, $entityName, $entityId, $isActive);
...
//            $result = $manager->hidrateResult(Entity::MEDIA, ['id' => $media->getId()])[0];
    ...
    public function fileSendAceptMultipleAction($type, $entityName, $entityId, $isActive) {
        ...
        if (!empty($_FILES['file'])) {
            $file = $_FILES['file'];
            try {
                $media = $this->get('bc.media.manager')->asyncFileSendAceptMultiple($file, $type, $entityName, $entityId, $isActive);
            } catch (\Exception $e) {
                return $manager->renderJson(['fail' => $e->getMessage()]);
            }
...
//            $result = $manager->hidrateResult(Entity::MEDIA, ['id' => $media->getId()])[0];
    ...
    public function fileSendAppAction(Request $request) {
        ...
        if (!empty($_FILES['file'])) {
            ...
            $target_path = $this->get('bc.media.file.uploader')->getTargetDir();
            $filename = md5(uniqid()) . '.' . $extension;
            $target_path = $target_path . '/' . $filename;
            try {
                move_uploaded_file($_FILES['file']['tmp_name'], $target_path);
                $media = $this->get('bc.media.manager')->fileSendApp($filename, $request->get('type'), $request->get('entityName'), $request->get('entityId'), $request->get('isActive'));
            } catch (\Exception $e) {
                return $manager->renderJson(['fail' => $e->getMessage()]);
            }
