...
class MailManager {
    ...
    public function sendEmail($emailTemplate, $parameters, $to, $from = null, $locale = null, $fromName = null, $cc = null, $bcc = null) {
        if (empty($locale)) {
            $locale = $this->container->get('av.manager')->getRequest()->getLocale();
        }
        if (empty($fromName)) {
            $fromName = $this->container->getParameter('av.conf.default.nombre.admin');
        }
        if (empty($from)) {
            $from = $this->container->getParameter('mailer_user');
        }

        $twig = $this->container->getParameter('av.email.directory') . ':' . $locale . '/' . $emailTemplate . '.html.twig';
