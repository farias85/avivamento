...
class DocumentoController extends Controller {
    ...
    public function fileSendAction($type, $entityName, $entityId, $isActive, $toEntityId, $toEntityName) {
        ...
        if (!empty($_FILES['file'])) {
            $file = $_FILES['file'];
            try {
                $documento = $this->get('av.documento.manager')->asyncFileSend($file, $type, $entityName, $entityId, $isActive, $toEntityId, $toEntityName);
            } catch (\Exception $e) {
    ...
    public function fileSendAceptMultipleAction($type, $entityName, $entityId, $isActive, $toEntityId, $toEntityName) {
        ...
        if (!empty($_FILES['file'])) {
            $file = $_FILES['file'];
            try {
                $documento = $this->get('av.documento.manager')->asyncFileSendAceptMultiple($file, $type, $entityName, $entityId, $isActive, $toEntityId, $toEntityName);
            } catch (\Exception $e) {
                return $manager->renderJson(['fail' => $e->getMessage()]);
            }
